...
def process_visualise_image(image_np):
    ...
    xmin, ymin, xmax, ymax = xmin * image_array.shape[1], ymin * image_array.shape[0], xmax * image_array.shape[1], ymax * image_array.shape[0]

    cv2.rectangle(image_np, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (0, 0, 255), 2)  # Crveni okvir (BGR format)

    # Dodajte ime klase
    class_name = label_names[cls]
    cv2.putText(image_np, f'{class_name} {prob * 100:.2f}%', (int(xmin), int(ymin) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
...
def detect_objects_in_live_video():
    ...
    fps = 10
    video_writer = cv2.VideoWriter('live_annotated_video.mp4',
